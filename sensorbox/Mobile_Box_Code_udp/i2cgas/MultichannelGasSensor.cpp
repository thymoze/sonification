...
float MutichannelGasSensor::calcGas ( int gas, bool useCache )
{
   ...
   //##############################################################################################################################################
   //printf("%i , %f , %f ", An_0, ratio0, ratio2);
...
** Returns:
                            float value - concentration of the gas
*********************************************************************************************************/
int MutichannelGasSensor::readRawGas ( int gas, bool useCache )
{

    int ratio0, ratio1, ratio2;
    if(1 == __version)
    {
        if(readR() < 0)
            return -2.0f;
   ...
   //printf("%i , %f , %f ", An_0, ratio0, ratio2);
    ...
    switch(gas)
    {
        case CO:
        {
            c = ratio1;  //mod by jack
            break;
        }
        case NO2:
        {
            c = ratio2;  //mod by jack
            break;
        }
        case NH3:
        {
            c = ratio0;  //modi by jack
            break;
        }
        case C3H8:  //add by jack
        {
...
//        printf ( "%s \t %20.8f ppm (%s)\n", gases [g], v, gasNames [g] );
        ...
        printf ("{\"name\" : \"%s\", \"value\" :%20.8f, \"raw\" : %i}",gases[g], v, u);
